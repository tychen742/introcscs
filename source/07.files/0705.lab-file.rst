...
Below is a more elaborate, complete example,
...
Note that ``sum_file.cs`` uses the ``UI`` class. We can simply copy the file 
...
a.  Copy :file:`../../examples/introcs/sum_file/sum_file.cs` to a file :file:`safe_sum_file.cs` in a new project of yours. 
