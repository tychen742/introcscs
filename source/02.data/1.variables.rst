...
data values data values (objects) somewhere else. 


In C#, basic dta types include ``string``, ``char``, ``bool``, ``int``, and ``decimal``:abbr:

- ``int`` for integers, 
- ``double`` for numbers allowing a fractional part, approximating more general real numbers.  There are many other numeric types and also non-numeric types, but we can use 
- ``int`` 
