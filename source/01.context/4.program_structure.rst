...
#. A ``bin`` directory will be created later after we build and run the application.
...
#. ``namespace``: 
   ...
   that these types (class, interface, struct, ...) are not **value types** such 
   as int, float, and string as predefined by .NET for us, but **reference types**.  
...
#. The ``Main`` method:
   The most common entry point of a C# program is static void Main() or static 
   void Main(String []args). The Main method is the entry point of a program and 
   therefore the first method invoked when an application is executed. 
