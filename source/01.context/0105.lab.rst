...
#. Create a C# Console app project folder
   ...
   Here you will create a folder for a test project, call it **Hello** or whatever you like, 
   ...
   #. Change directory (``cd``) into the **tests** directory: The tests directory is inside your 
      workspace directory, so we have to change into workspace first (``cd workspace``) then ``cd tests``. 
   #. Make sure you are in *tests*: ``pwd`` + Enter and you should see that you are in the tests directory in workspace/tests. 
   ...
   #. Check your location: Issue the ``pwd`` command and you should be in the Hello project directory. 
...
#. Start VS Code from terminal and run the project

   While we usually (and will in the future!) start VS Code by clicking on the application icon, for now, 
   follow the steps below: 
   
   #. Change into the project directory. 
   #. Start VS Code by issuing the command ``code .`` (``code`` is VS Code and the ``.`` means the present location).
   #. VS Code will start with the folder as the working directory shown in the **Explorer** view.
