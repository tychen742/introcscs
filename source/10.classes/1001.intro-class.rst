...
When it comes to customizing a ``Print()`` method to print out strings, a good design 
decision is to separate the different actions: the first is to generate the showing 
the full state of the object. Once we have this string, we can easily print it or 
write it to a file. Hence we want a method to generate a descriptive string.
...
too. How can the compiler know how to handle types that were *not invented* when the compiler was written?

The answer is to have common features among all objects.  
All object in C# have a ``ToString`` method because all types inherit from System.Object, 
and that method is used implicitly when an object is used with string concatenation, and also for ``Write``.
The default version supplied by the system is not very useful for a new object 
that it knows nothing about! You need to define your own version, 
one that knows how you have defined your type, with its own specific instance variables. 
...
See what the method does: it uses the object state (e.g., the values of variables name, phone, and 
email in this case) to create and *return* a single string representation of the object.  
