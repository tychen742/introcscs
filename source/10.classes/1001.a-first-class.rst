...
Some important basic concepts about class and object-oriented programming (OOP) include: 
...
- **Class Instance**: An ``object`` is an ``instance`` of a class. Namely, an object is created based on a 
  class and is an instance of that class. An object can be created from a class using the ``new`` keyword. 


There are related terminology that you will see about class and OOP:

- **Field**: In C#, a field stores a piece of data within an object. It acts like a variable and may have a different 
  value for each instance of a type. [#field-property]_
...
- Class Identifier: An identifier is the name you assign to a type (class, interface, struct, delegate, or enum), 
  member, variable, or namespace. An identifier begins with a letter or underscore. Use PascalCase for class names and method names.
- Class Body: The class body is surrounded by { }. 
- Base Class or Super Class: Optional. Required when this class is the 
- Interfaces: 
...
C# comes with lots of built-in data types, but not everything we might want to use. That is why we build classes to 
represent the types. Assume that we want to build a code template (class) for managing **Contact** information that 
involves several pieces of data, and they are all unified by the fact that they are for one person, so we would 
like to store them together as a unit (a class).  

For simplicity, let us just consider the contact information to be
name, phone number, and email address. You could always keep three independent string variables, 
...
We have already considered built-in types with internal state (values stored in memory), like a ``List``:   
...
Thus far we have stored data as local variables, and we have called methods,
with two ways to get information in and out of methods:
...
a new class that we write. Now we have the idea of an object that has *internal* state
(like a contact with a name, phone, and email).  We shall see that
this state is *not* stored in local variables and does *not* need to be passed 
through parameters for methods *within* the class. Pay careful attention as we introduce 
this new location for data and the new ways of interacting with it.
...
We declare these *in* the class and *outside* any method declaration. They are 
*fields* of the class.  As we will discuss more in terms of safety and security, 
