...
#. fake_help_verbose ():repsrc:`fake_help_verbose`) (Change the ``Main``() to something else such as ``main`` as you are allowed to have only one Main in a project.)
#. Also, the UI class (``ui.cs`` at :repsrc:`ui`) so you don't have to write the user input code. 

Note that there are data files (\*.txt) in the project directory. 

This lab provides a file :repsrc:`fake_help.cs <dict_lab_stub/fake_help.cs>` for an project. The project still needs 
...
Look in your copy of ``fake_help.cs`` (where the project ``Main()`` method resides) :repsrc:`fake_help.cs``: 

- It creates the ``List`` ``guessList`` and the ``Dictionary`` ``responses`` 
  using more general functions that you need to fill in.
- The stubs for these new versions are put in the class ``FileUtil`` for easy reuse. 
...
This is the same as method tracing except it happens across files).
...
definitions in the class ``FileUtil``.  Look back to ``Main`` in ``fake_help`` to
