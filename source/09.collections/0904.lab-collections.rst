...
``List`` and ``Dictionary``. You will later provide simple versatile methods to
...
For now, run the program to see how this fake AI/virtual would look like:
...
Look back into the code, in the ``Main`` method, you see that:: 
   ...
   public static void Main(string[] args)
      {
         List<string> guessList = GetParagraphs();                ### create a filled string list
         Dictionary<string, string> responses = GetDictionary();  ### create a filled dictionary
   ...
   What do you have to say?");                                       ### the @ means the string is a verbatim string literal; escape sequence is ignored
         ...
         do 
         {                                                        ### use do-while loop to loop at least once and keep looping
            ...
            if (fromUser != "bye") {                              ### condition required here because this is first run
               string answer = Response(fromUser, guessList, responses);  ### throw the args to Response()
...
The ``Response`` method
                                    ...
                                    Dictionary<string, string> responses)
   {
      ...
      // if the user does not input a word in "response", guess
