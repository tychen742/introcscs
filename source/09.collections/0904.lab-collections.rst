...
Copy project files from :repsrc:`dict_lab_stub` to your own project:
...
Note that there are data files (\*.txt) in the project directory. Also, download the UI class (:repsrc:`dict_lab_stub`) 
into the same project folder so you don't have to write the user input methods. 
