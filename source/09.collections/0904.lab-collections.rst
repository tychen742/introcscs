...
hard-coded in the program and that that is not ideal for maintenance. Your team want to use external text 
files so that we have manage/add response texts with ease. For that your colleagues have prepared a file 
...
Open the working program downloaded take_help_verbose.cs (:repsrc:`fake_help_verbose/fake_help_verbose.cs`) and
