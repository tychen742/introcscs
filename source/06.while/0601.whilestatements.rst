...
and conditional statements: _[#flowchart-while-loop]
...
Because that condition (boolean expression) is evaluated before each execution of the loop, 
a while loop can executes **zero** or more times. 

As an example, you should test the following code in ``csharprepl`` to get familiar with 
C# while loop::

   > int n = 0;                  // the condition
