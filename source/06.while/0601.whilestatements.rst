...
the sequential flow of a program is altered with loops, just like with method calls
...
Because that expression is evaluated before each execution of the loop, 
a while loop can executes **zero** or more times. 

As an example, you should test the following code in ``csharprepl``::
