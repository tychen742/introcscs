...
The comment before the method definition does not give a clear idea of the 
range of possible values for ``n``.  How small makes sense for the comment?
What actually works in the method?  The smallest expression 
starting with 1 would just be 1: (``n`` is 1).  Does that work in the method?
You were probably not thinking of that when developing the method!
...
and the body of the loop *never* runs (2 <= 1 is false).  The method
...
Recall that *when a return statement is reached, method execution ends*,
...
6, the method terminates and never loops back, returning 3.
...
method following Gauss's model.  Here we introduce the variable average,
...
Here is a shot at the whole method:
...
Further analysis:  To make sure the method always worked, it made sense
to leave the parameter ``n`` an ``int``.  The method would not work
